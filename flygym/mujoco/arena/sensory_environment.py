...
class OdorArena(BaseArena):
    ...
    key_angle : bool
        This booleans tells the arena how to compute the key
        used in the valence dictionary for the different smells
    """
    ...
    def __init__(
        ...
        if key_angle:
            # Compute the angle key for each smell and update the internal valence dictionary
            for i in range(self.num_odor_sources):
    ...
    def normalize_peak_intensity(self, peak_intensity):
        """
        This function is used to normalize the different peak intensity.

        Input - peak intensity: [n, k]
        """
        return peak_intensity / np.linalg.norm(peak_intensity)

    def compute_smell_angle_value(self, peak_intensity):
        """
        Method to compute the key for the different peak intensity
        by choosing as key the angle in the complex plane
        generated by the [x,y] components of the smell source.
    ...
    def generate_random_gains(self, explore=True):
        ...
        Parameters
        explore : bool
            Whether the fly is free to explore or it is guided to a specifif source
        """
    ...
    def generate_curiosity_rdm_gains_food_internal_state(self):
        """
        Method to compute random gains for the odor sources.
        The range of gains is [0, 500].
    ...
    def generate_exploration_turning_control(self, attractive_gain, aversive_gain, obs):
        ...
        and attractive/aversive sources.
        """
    ...
    def generate_specific_turning_control(
        ...
        the fly needs to approach a source specified
        by the index_source.
        """
