...
class OdorArena(BaseArena):
    ...
    def generate_random_gains(self, explore=True) -> Tuple:
        ...
        ----------
        explore : bool
            Whether the fly is free to explore or it is guided to a specific source
        ...
        match internal_state:
            ...
            case "starving":
                ...
                for i in len(self.odor_source):
                    ...
                    fly_pos - self.odor_source[i, :2]
                )
                    if tmp_distance < distance:
                        distance = tmp_distance
                        index_source = i
                # If food source is attractive
                if self.peak_odor_intensity[index_source][0] > self.peak_odor_intensity[index_source][1]:
                    attractive_gain = -max(x, y)
                    aversive_gain = 0
                # Else it's aversive
                else:
    ...
    def run_odor_taxis_simulation(
        ...
        num_decision_steps : float
            Number of decision steps
        obs : ObsType
            The observation as defined by the environment.
        physics_steps_per_decision_step : float
            Number of physics steps per decision step
        odor_history : np.array
            The observation[odor_intensity].
        obs_hist : ObsType
            The observation as defined by the environment.
    ...
    def generate_exploration_turning_control(self, attractive_gain, aversive_gain, obs) -> np.array:
        """
        ...
        or attractive/aversive sources.
